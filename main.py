...
def _run_domestic_cycle(ctx: dict, token: str) -> int:
    ...
    for stock_code, info in list(holdings.items()):
        ...
        # 손절: 매입가 대비 stop_loss_pct% 이상 하락 시 매도
        ...
        if stop_loss_pct > 0 and avg_price > 0:
            ...
            if drop_pct <= -stop_loss_pct:
                ...
                logger.info(f"국내 손절 매도: {label} | 매입가: {avg_price:,.0f} | 현재가: {current_price:,.0f} | 하락률: {drop_pct:.2f}%")
                continue
    ...
    for symbol, info in list(holdings.items()):
        ...
        if stop_loss_pct > 0 and avg_price > 0:
            ...
            if drop_pct <= -stop_loss_pct:
                ...
                logger.info(f"해외 손절 매도: {label} | 매입가: ${avg_price:.2f} | 현재가: ${current_price:.2f} | 하락률: {drop_pct:.2f}%")
                continue
        ...
        for stock_code, info in list(holdings.items()):
            ...
            if drop_pct <= -config.stop_loss_pct:
                ...
                logger.info(
                    ...
                    f"현재가: {current_price:,.0f} | 하락률: {drop_pct:.2f}%"
                )
